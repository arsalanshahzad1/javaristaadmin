...
// Roles that read better as acronyms / special-cased labels than plain title case.
...
// Title-cases snake_case values and special-cases acronym roles: 'store_manager' -> 'Store Manager', 'ceo' -> 'CEO'.
...
// Fetch every user matching the current filters (across all pages) for CSV export.
async function fetchAllUsers(params: { search?: string; role?: Role }) {
  const pageSize = 100;
  const first = await getUsers({ ...params, page: 1, limit: pageSize });
  const all: AdminUser[] = [...(first.data ?? [])];
  const totalPages = first.pagination?.pages ?? 1;

  for (let currentPage = 2; currentPage <= totalPages; currentPage += 1) {
    const next = await getUsers({ ...params, page: currentPage, limit: pageSize });
    all.push(...(next.data ?? []));
  }
...
export function UsersManagementPage() {
  ...
  const [editingRole, setEditingRole] = useState<Role>('trainee');
  ...
  function startEditing(user: AdminUser) {
    ...
    setEditingInvestorLevel(user.investorAccessLevel || '');
  }
  ...
  async function handleExport() {
    ...
    try {
      ...
      toast.success(`Exported ${allUsers.length.toLocaleString()} users`);
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[#777]">Manage admin access, premium status, and employee performance reviews.</p>
        </div>
        <button
          type="button"
          onClick={handleExport}
          disabled={isExporting}
          className="inline-flex items-center gap-2 rounded-lg border border-[#333] px-4 py-2 text-sm font-medium text-[#ddd] hover:bg-[#242424] disabled:cursor-not-allowed disabled:opacity-50"
        >
          <Download size={16} />
                    ...
                    <tr key={id} className={`${index % 2 === 0 ? 'bg-[#171717]' : 'bg-[#1E1E1E]'} border-b border-[#242424]`}>
                        ...
                        {isEditing ? (
                            ...
                            <div className="flex items-center gap-1">
                              <select
                                ...
                                onChange={(event) => {
                                  ...
                                  // Non-investor roles save immediately. Investor requires an access
                                  // level, so we surface the sub-panel and wait for the Save click.
                                  if (newRole !== 'investor') {
                              ...
                              <button
                                ...
                                className="rounded border border-[#333] px-2 py-1 text-xs text-[#999] hover:bg-[#242424]"
                              >
                                Cancel
                              </button>
                            </div>
                            {editingRole === 'investor' && (
                              <div className="flex gap-1">
                                <select
                                  value={editingInvestorLevel}
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    updateRoleMutation.mutate({
                                      ...
                                      role: 'investor',
                                      isPremium: true,
                                      investorAccessLevel: editingInvestorLevel || undefined,
                                    });
                                  }}
                                  className="rounded border border-[#D62B2B] bg-[#D62B2B]/10 px-2 py-0.5 text-xs text-[#D62B2B] disabled:opacity-50"
                                >
                                  Save
                                </button>
                              </div>
                          ...
                          <div className="flex flex-col gap-0.5">
                            ...
                            {user.role === 'investor' && user.investorAccessLevel && (
                              <span className="text-xs text-[#777]">{formatRole(user.investorAccessLevel)}</span>
                            )}
                          </div>
                        )}
                      </td>
                      {/* Employee Role assignment — only relevant for employee-tier users. Reads
                          straight from server state and relies on invalidation after each change. */}
                      {/* <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-2">
                          <button type="button" onClick={() => startEditing(user)} className="rounded-lg border border-[#333] px-3 py-1.5 text-xs font-medium text-[#ddd] hover:bg-[#242424]">
                            Change Role
                          </button>
                          {(EMPLOYEE_TIER_ROLES as string[]).includes(user.role) && (
